...
func GetState(storeName, key string) ([]byte, error) {
	daprClient, err := client.NewClient()
	if err != nil {
		return nil, errors.Wrap(err, "new hddapr client")
	}
	if daprClient == nil {
		return nil, errors.New("hddapr client is null, handlerName resolution service may not started, please check it")
	}
